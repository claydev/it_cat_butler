...
local action = function(msg, blocks, ln)
    ...
    if blocks[1] == 'goodbye' then
        ...
        -- ignore if not input text and not reply
        if not input and not msg.reply then
            api.sendReply(msg, make_text(lang[ln].settings.goodbye.no_input), false)
            return
        end
        ...
        if not input and msg.reply then
            ...
            if replied_to == 'sticker' or replied_to == 'gif' then
                ...
                db:hset(hash, 'type', 'media')
                db:hset(hash, 'content', file_id)
                api.sendReply(msg, lang[ln].settings.goodbye.media_setted..'`'..replied_to..'`', true)
            else
                api.sendReply(msg, lang[ln].settings.goodbye.reply_media, true)
            end
            return
        end
		...
		db:hset(hash, 'type', 'custom')
		db:hset(hash, 'content', input)
		local res, code = api.sendReply(msg, make_text(lang[ln].settings.goodbye.custom, input), true)
		if not res then
			db:hset(hash, 'type', 'composed') --if wrong markdown, remove 'custom' again
			db:hset(hash, 'content', 'no')
			if code == 118 then
			...
			local id = res.result.message_id
			api.editMessageText(msg.chat.id, id, lang[ln].settings.goodbye.custom_setted, false, true)
		end
    end
