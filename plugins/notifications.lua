local function scan_mentions(msg, ln)
	--vardump(msg)
			...
			if user_id then
				...
				if db:hget(hash, user_id) == 'yes' then
					...
					if msg.chat.username then
						...
						text = make_text(lang[ln].notifications.mention1, msg.from.first_name:mEscape(), link, msg.chat.title:mEscape())
					else
						text = make_text(lang[ln].notifications.mention2, msg.from.first_name:mEscape(), msg.chat.title:mEscape())
					end
...
local function control(msg, blocks, ln)
	--vardump(msg)
	--vardump(blocks)
